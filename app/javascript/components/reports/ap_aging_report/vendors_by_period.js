        ...
        <Table>
          <TableHead>
            <TableRow>
              ...
              <TableCell className={classes.headerRight}><BalanceCell balance="Amount" /></TableCell>
              <TableCell className={classes.headerRight}><BalanceCell balance="Open Balance" /></TableCell>
            </TableRow>
          </TableHead>

          <TableBody>
            {periodNames.map(period => {
              const vendorPeriod = Object.entries(vendorsByPeriod[period]);
              return (
                <Fragment key={period}>
                  {(!showActiveRows || vendorPeriod.length > 0) && (
                    <TableRow key={period}>
                  ...
                  {vendorPeriod.length === 0 && !showActiveRows && (
                  ...
                  {vendorPeriod.map((accountGroup, index) => {
                    const [code, invoices] = accountGroup;
                    return (
                      <Fragment key={index}>
                        <TableRow key={period}>
                          <TableCell
                          ...
                          >
                            {code} &mdash; {invoices[0].fund_name}
                          </TableCell>
                        </TableRow>
                        {invoices.map(
                          (
