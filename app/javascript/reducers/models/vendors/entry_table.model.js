  ...
  reducers: {
    ...
    setNumberFilter: (state, number) =>
      produce(state, (draft) => {
        ...
        draft.filter.number = number;
      }),
    setInvoiceNumberFilter: (state, invoice_number) =>
      produce(state, (draft) => {
        if (!state.filter) {
          draft.filter = {};
        }
        draft.filter.invoice_number = invoice_number;
      }),
    setPaidFilter: (state, paid) =>
      produce(state, (draft) => {
        if (!state.filter) {
          draft.filter = {};
        }

        draft.filter.paid = paid;
      }),
  },
