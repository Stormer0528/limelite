...
// Components
import ShowView from "../components/vendors/show_view";
...
import routes from "../components/vendors/routes";
...
const generateClassName = createGenerateClassName({
  productionPrefix: "vendor-show",
  seed: ["vendor-show", Math.random()].join("-"),
});
...
const renderComponent = (Component) => {
  const elem = document.getElementById("vendor-show-view");
  if (elem) {
    const {fiscalYear} = elem.dataset;
    store.dispatch({
      type: "entryFilter/setAfterDateFilter",
          ...
          <ApolloProvider client={client}>
            <StylesProvider generateClassName={generateClassName}>
              <ThemeProvider theme={siteTheme}>
                <Router routes={routes} basename="/vendors">
...
document.addEventListener("DOMContentLoaded", () => {
  renderComponent(ShowView);
});

// Update from HMR Calls
if (module.hot) {
  module.hot.accept("../components/vendors/show_view", () => {
    renderComponent(ShowView);
  });
}
