# == Schema Information
#
# Table name: report_monthly_profit_loss_statements
#
...
# Indexes
#
#  index_report_monthly_profit_loss_statements_on_organization_id  (organization_id)
#
# Foreign Keys
#
...
class Report::MonthlyProfitLossStatement < ApplicationRecord
  ...
  def run_report(_should_save=true)
    ...
    setup do
      ...
      @default_row = [*db_titles.map {|code| [code,	"$0.00"] }, ["balance", "$0.00"]].to_h.freeze
  ...
  def fields_for_account_objects(object_codes:, fund_code:)
    row = @totals.where(fund_code: fund_code, object_code: object_codes).group(
      :object_code, :description
    ).select("object_code AS code").select(:description).select("CAST(SUM(total_in_cents) AS BIGINT) AS \"balance\"")

    db_titles.each do |month_code|
      row = row.select("CAST(SUM(\"#{month_code}\") AS BIGINT) AS \"#{month_code}\"")
    end
