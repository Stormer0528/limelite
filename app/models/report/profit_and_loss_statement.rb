...
class Report::ProfitAndLossStatement < ApplicationRecord
  ...
  def run_report(_should_save=true)
    ...
    setup do
      ...
      @default_row = [*@fund_codes.map {|fund_code| [fund_code, "$0.00"] }, ["balance", "$0.00"]].to_h.freeze
    end
  ...
  def titles
    ["Object", "Description", *data[:account_funds].pluck(:code), "Balance"]
  end
  ...
  def fields_for_account_objects(object_codes:)
    ...
    @fund_codes.each do |code|
      row = row.select("CAST(SUM(CASE WHEN \"fund_code\" = '#{code}' THEN total_in_cents ELSE 0 END) AS BIGINT) AS \"#{code}\"")
    end
